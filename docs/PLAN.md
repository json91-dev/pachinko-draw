...
- **입력 영역 (화면 하단):**
  - "이름들을 입력하세요" 제목
  - **레이아웃:** textarea(좌, 가장 큰 면적) + 우측 컬럼(맵 드롭다운 위 / 시작 버튼 아래) 가로 배치
  - 큰 textarea (콤마 구분 닉네임 입력)
  - 맵 선택 `<select>` 드롭다운 (기본맵 / 빙글빙글 물레방아) — textarea 우측 상단
  - "시작" 버튼 (유효성 통과 시 활성화) — 드롭다운 아래
- **유효성 검사:** 빈 닉네임 불허, 중복 불허, 2~30명
- **전환:** 시작 버튼 클릭 → 구슬 즉시 발사 + 입력 영역 즉시 제거 → 게임 Canvas 전환
...
- **형식:** 각 플레이어 한 행, 우측 정렬
  ...
  10개 닉네임1 #2   ← 점수 2위
  ```
- **정렬:** 점수 내림차순 실시간 정렬 — 점수가 높은 플레이어가 위로 올라오고 #1/#2/#3 순위 번호도 동적으로 변경
- **색상:** 행별 텍스트 색상 = 해당 플레이어의 구슬 색상 (순위가 바뀌면 색상도 함께 이동)
- **설정 화면:** 구슬 수 = 분배된 초기 구슬 수 (동점이므로 정렬 순서 = 입력 순서)
- **게임 화면:** 구슬 수 = 현재 점수(구멍에 넣은 수)로 실시간 업데이트 + 내림차순 재정렬
- **overflow:** `overflow: hidden` — 화면 영역을 넘는 항목은 잘림 처리, 스크롤 없음
...
- 렌더링: `ctx.shadowBlur = 12`, `ctx.shadowColor = '#22FFFF'`로 glow
...
- 핀 배치: 기본맵과 동일한 행 간격 유지. 물레가 위치하는 행(y좌표 겹치는 행)만 핀 제외, 나머지 행은 동일하게 배치
...
- "BLACK HOLE MODE" 텍스트: 화면 우측 상단, 빨간색, 큰 글씨 (warning 스타일)
  - CSS 또는 Canvas로 5회 깜빡임(blink) 후 고정 표시
...
- 구슬 소진 또는 우승 확정 시 → 줌 원래 배율로 복귀(scale 2.5 → 1.0) → 복귀 완료 후 WinnerDisplay + 컨페티 표시
- 줌인 중 조기 우승 확정되더라도 동일하게 줌 복귀 후 표시
...
- **새 게임 시작:** 시작 버튼 클릭 시 기존 Matter.js 엔진 즉시 초기화 후 새 게임 시작
